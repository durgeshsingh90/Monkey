...
def read_file_with_fallback(file_path):
    ...
    for enc in encodings:
        try:
            logger.debug(f"Trying to read file: {file_path} with encoding: {enc}")
            with open(file_path, 'r', encoding=enc) as file:
                return file.readlines()
        except UnicodeDecodeError:
            logger.debug(f"Failed to read file: {file_path} with encoding: {enc}")
            continue
...
def get_total_parts(file_path):
    ...
    while True:
        ...
        part_number += 1
    logger.debug(f"Total parts found: {part_number}")
    return part_number
...
def process_html_file(file_path, total_parts):
    for part_number in range(total_parts):
        ...
        logger.debug(f"Reading part file: {part_file_name}")
        lines = read_file_with_fallback(part_file_name)

        # Check if the lines are empty (meaning there's nothing to process)
        if not lines:
            logger.debug(f"No content in {part_file_name}, stopping processing.")
            break
        ...
        if found_br_tag:
            logger.debug(f"Found <BR></BR> in {part_file_name}. Snatched lines: {len(parts_to_append)}")
            ...
            with open(next_part_file_name, 'w', encoding='utf-8') as next_part_file:
                logger.debug(f"Writing to next part file: {next_part_file_name}")
                # Write <BR></BR> first
            ...
            with open(part_file_name, 'w', encoding='utf-8') as original_file:
                logger.debug(f"Rewriting original part file: {part_file_name}")
                original_file.writelines(remaining_lines)  # Rewrite the original part file without the copied lines
        else:
            logger.debug(f"No <BR></BR> found in {part_file_name}.")
...
#     try:
#         logger.info(f"Processing main file: {main_file_path}")
#         total_parts = get_total_parts(main_file_path)
#         process_html_file(main_file_path, total_parts)
#         logger.info(f"Processing completed for file: {main_file_path}")
...
#     main()
# Replace `main()` with:
def run_adjusthtml(main_file_path):
    try:
        logger.info(f"Processing main file: {main_file_path}")
        total_parts = get_total_parts(main_file_path)
        process_html_file(main_file_path, total_parts)
