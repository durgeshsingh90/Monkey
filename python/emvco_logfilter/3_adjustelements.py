...
# Prepend content to all part files except part0
for part_file in other_parts:
    prepend_content(part_file, lines_to_copy)

# Append content to all part files except the last part file
for part_file in [part0_file] + other_parts[:-1]:
    append_content(part_file, lines_to_append)

print("Content prepended from part0 and appended from the last part successfully, excluding same files.")
